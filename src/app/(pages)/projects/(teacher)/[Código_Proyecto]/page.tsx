...
// //import { Space } from "@/types/Space";  // Asegúrate de que esta ruta sea correcta
...
//                     // Cambia el tipo de `onNewSpace` para que espere un objeto de tipo `Space`
//                     <NewSpace
//                         params={{ Código_Proyecto: project.Código_Proyecto }}
//                         onNewSpace={(space: Space) => handleNewSpace(space)} // Aquí ya no se usa Partial<Space>
//                     />
...
export default function ProjectPage({ params }: ProjectPageProps) {
    ...
    const spacesWithUserDetails: SpaceWithUser[] = projectSpaces.map(space => {
        const userDetail = users?.find(user => user.ID_Usuario === space.ID_Usuario);
        return {
            ...space,
            Usuario: userDetail || null,
    ...
    const handleNewSpace = (space: Space) => {
        addSpace(space);
    };
        ...
        <section className="flex flex-col gap-y-8">
                                ...
                                <CardBody className="overflow-visible p-0 w-full h-full">
                                    <FileUpload
                                        ...
                                        existingFile={{
                                            ...
                                            url: `${storageUrl}/${document.Ruta_Documento}`
                                        }}
                                        readOnly={true}
                                        className="h-full w-full"
                                    />
            ...
            <section className="flex flex-wrap gap-4 p-4">
                {isLoadingSpaces ? (
                    ...
                    spacesWithUserDetails.map(space => (
                        <Link
                            href={`${project?.Código_Proyecto}/${space.Nombre_Espacio}`}
