...
import { obtenerEquiposPorUsuario } from '../../../../api/register.api';
...
const InfoPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLatestTeamData = async () => {
      const response = await obtenerEquiposPorUsuario(15);
      console.log("dato", response.data);
      if (response && response.length > 0) {
        const latestTeam = response[response.length - 1]; // Obtener el último equipo
        setProjectData(latestTeam);
        console.log("dato2", projectData);
      }
    };
    fetchLatestTeamData();
  }, []);
